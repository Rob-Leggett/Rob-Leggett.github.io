...
tags: ["Google Cloud", "Data Loss Prevention", "DLP API", "Machine Learning", "Image Recognition", "Security", "Data Privacy", "Sensitive Data Detection", "Robert Leggett"]

meta:
  title: "Why Google’s Data Loss Prevention API Isn’t Ready for Image Detection | Robert Leggett"
  description: "An in-depth review of Google Cloud’s DLP API image detection capability. Explore real test cases, results, and limitations when detecting sensitive data like credit card numbers in images."
  keywords: ["Google DLP API", "Data Loss Prevention", "Image Redaction", "Credit Card Detection", "Google Cloud Security", "Sensitive Data Masking", "Machine Learning", "Data Protection", "Robert Leggett"]
  author: "Robert Leggett"
  canonical: "https://robertleggett.com.au/blog/why-i-dont-think-googles-data-loss-prevention-api-is-ready-for-use"
  image: "https://robertleggett.com.au/blog/why-i-dont-think-googles-data-loss-prevention-api-is-ready-for-use/feature-image.png"

og:
  type: "article"
  title: "Why Google’s Data Loss Prevention API Isn’t Ready for Image Detection"
  description: "Through real-world tests, this article explores why Google Cloud’s DLP API struggles with consistent image-based sensitive data detection, and what’s needed before it’s production-ready."
  image: "https://robertleggett.com.au/blog/why-i-dont-think-googles-data-loss-prevention-api-is-ready-for-use/feature-image.png"
  url: "https://robertleggett.com.au/blog/why-i-dont-think-googles-data-loss-prevention-api-is-ready-for-use"

twitter:
  card: "summary_large_image"
  title: "Why Google’s DLP API Isn’t Ready for Image Use"
  description: "A deep dive into Google Cloud’s Data Loss Prevention API and its image detection limitations. Includes Java examples, test results, and recommendations for developers."
  image: "https://robertleggett.com.au/blog/why-i-dont-think-googles-data-loss-prevention-api-is-ready-for-use/feature-image.png"
---
...
*   A breakdown of the code used to execute Google's Data Loss Prevention API
...
Ensure you set up the required authentication of the service to allow for execution against your Google account: **export GOOGLE_APPLICATION_CREDENTIALS="/user//\[service-account-key\].json"**
...
public class InspectImageFile {
    ...
    private static void deIdentifyWithMask(
        ...
        try (DlpServiceClient dlpServiceClient = DlpServiceClient.create()) {
            ...
            // Print the masked result (e.g. "My SSN is 123456789" → "My SSN is *********")
    ...
    public static void redactImageFile(String projectId, String filePath, String outputFilePath) {
        try (DlpServiceClient dlp = DlpServiceClient.create()) {
            ...
            // Save redacted file
            try (FileOutputStream redacted = new FileOutputStream(outputFilePath)) {
            ...
            System.out.println("Error during redactImageFile:\n" + e);
        }
    }
}
```
...
public static void inspectImageFile(String projectId, String filePath) {
    ...
    // After completing all of your requests, call the "close" method on the client
    ...
    try (DlpServiceClient dlp = DlpServiceClient.create()) {
        ...
        // Specify the type of info the inspection will look for.
        List<InfoType> infoTypes = new ArrayList<>();
        // See https://cloud.google.com/dlp/docs/infotypes-reference for a complete list of info types.
        for (String typeName : new String[]{"CREDIT_CARD_NUMBER"}) {
            infoTypes.add(InfoType.newBuilder().setName(typeName).build());
        }
...
private static void deIdentifyWithMask(
    ...
    try (DlpServiceClient dlpServiceClient = DlpServiceClient.create()) {
        ...
        // Create the de-identification transformation configuration
        ...
        // Create the de-identification request object
        DeidentifyContentRequest request = DeidentifyContentRequest.newBuilder()
                .setParent(ProjectName.of(projectId).toString())
                .setInspectConfig(inspectConfig)
                .setDeidentifyConfig(deidentifyConfig)
        ...
        // Execute the de-identification request
        DeidentifyContentResponse response = dlpServiceClient.deidentifyContent(request);
